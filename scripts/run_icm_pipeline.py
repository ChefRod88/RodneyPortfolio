...
def run_stage_1(workspace_root, job_desc, api_key):
    ...
    job_desc_path = os.path.join(workspace_root, "01_skill_extraction", "reference", "job_description.txt")
    if job_desc:
        safe_write(job_desc_path, job_desc)
    
    job_desc_content = safe_read(job_desc_path)
    if not job_desc_content:
        print("Error: Job description missing at 01_skill_extraction/reference/job_description.txt")
        sys.exit(1)
...
Please execute the process described in the Stage Contract. Provide only the Markdown response for extracted_requirements.md."""
    ...
    out_path = os.path.join(workspace_root, "01_skill_extraction", "working", "extracted_requirements.md")
...
def run_stage_2(workspace_root, api_key):
    ...
    s1_output_path = os.path.join(workspace_root, "01_skill_extraction", "working", "extracted_requirements.md")
    s2_ref_path = os.path.join(workspace_root, "02_resume_comparison", "reference", "extracted_requirements.md")
    ...
    resume_dest = os.path.join(workspace_root, "02_resume_comparison", "reference", "RodneyResume.md")
...
Please execute the process described in the Stage Contract. Provide only the Markdown response for comparison_results.md."""
    ...
    output = call_openai(prompt, api_key)
    out_path = os.path.join(workspace_root, "02_resume_comparison", "working", "comparison_results.md")
    safe_write(out_path, output)
...
def run_stage_3(workspace_root, api_key):
    ...
    s2_output_path = os.path.join(workspace_root, "02_resume_comparison", "working", "comparison_results.md")
    s3_ref_path = os.path.join(workspace_root, "03_interview_preparation", "reference", "comparison_results.md")
    ...
    # Copy reference docs
    shutil.copyfile(os.path.join(workspace_root, "_config", "skills", "RodneyResume.md"), 
                    os.path.join(workspace_root, "03_interview_preparation", "reference", "RodneyResume.md"))
    shutil.copyfile(os.path.join(workspace_root, "_config", "voice.md"), 
                    os.path.join(workspace_root, "03_interview_preparation", "reference", "voice.md"))
    
    global_identity = safe_read(os.path.join(workspace_root, "_config", "global_identity.md"))
    task_routing = safe_read(os.path.join(workspace_root, "_config", "task_routing.md"))
    contract = safe_read(os.path.join(workspace_root, "03_interview_preparation", "CONTEXT.md"))
    comparison_text = safe_read(s3_ref_path)
    resume_text = safe_read(os.path.join(workspace_root, "03_interview_preparation", "reference", "RodneyResume.md"))
    voice_text = safe_read(os.path.join(workspace_root, "03_interview_preparation", "reference", "voice.md"))
...
Please execute the process described in the Stage Contract. Provide the Markdown response for interview_guide.md and append the JSON block containing the structured output at the very end."""
    ...
    output = call_openai(prompt, api_key)
    out_path = os.path.join(workspace_root, "03_interview_preparation", "working", "interview_guide.md")
    safe_write(out_path, output)
